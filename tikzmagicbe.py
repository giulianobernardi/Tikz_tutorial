...
class TikzMagics(Magics):
    ...
    def _run_latex(self, code, dir):
        ...
        try:
            retcode = call("pdflatex -shell-escape tikzbe.tex", shell=True)
            if retcode != 0:
    ...
    def _convert_pdf_to_svg(self, dir):
        ...
        try:
            retcode = call("pdf2svg tikzbe.pdf tikzbe.svg", shell=True)
            if retcode != 0:
    ...
    def _convert_png_to_jpg(self, dir):
        ...
        try:
            retcode = call("convert tikzbe.png -quality 100 -background white -flatten tikzbe.jpg", shell=True)
            if retcode != 0:
    ...
    @line_cell_magic
    def tikzbe(self, line, cell=None, local_ns=None):
        '''
        Run TikZ code in LaTeX and plot result.
        
            In [9]: %tikzbe \draw (0,0) rectangle (1,1);
        ...
        The size and format of output plots can be specified::
            ...
            In [18]: %%tikzbe -s 600,800 -f svg --scale 2
                ...: \draw (0,0) rectangle (1,1);
                ...: \filldraw (0.5,0.5) circle (.1);
 
        TikZ packages can be loaded with -l package1,package2[,...]::
 
            In [20]: %%tikzbe -l arrows,matrix
        ...
        '''
        args = parse_argstring(self.tikzbe, line)
...
# can place custom packages here        
        tex = []
        tex.append('''
...
\\end{document}
        ...
        image_filename = "%s/tikzbe.%s" % (plot_dir, plot_format)
...
__doc__ = __doc__.format(
    TIKZ_DOC = ' '*8 + TikzMagics.tikzbe.__doc__,
    )
